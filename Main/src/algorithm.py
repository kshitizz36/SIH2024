...
# Reversed subsectors logic
def reversed_subsectors(G, current_node, target_node, distance_to_target):
    subsector_width = np.clip(distance_to_target / 10, 0.1, 1.0)  # Adjust subsector width based on distance to target
    return subsector_width

# Advanced A* implementation with all strategies from the paper
def astar_path(G, source, target, base_speed, start_time, ship_displ, ship_power, ship_load, ship_beam):
    ...
    while open_set:
        ...
        if current == target:
            ...
            return path[::-1], times[::-1], fuel_consumed  # Return reversed path, cumulative times, and fuel consumed
...
def process_neighbor(G, current, neighbor, target, base_speed, ship_displ, ship_power, ship_load, ship_beam):
    ...
    distance = haversine(current_pos, neighbor_pos)
